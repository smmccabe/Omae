...
function setupSkills() { //Well, this is used for more then just skills now. It should only run once at start up
  ...
  //@TODO - camelcase properly
  //@TODO - name parameters properly
  function spellattselect(x, y) { //so people can select which attribute is used for the increase/decrease spells
  ...
  for (var item in weapons) { //this pulls the weapons stats from the weapons object to populate the page
    ...
    for (var firemode in itemhold["mode"]) {
      $(".modes." + item).after(itemhold["mode"][firemode] + ",<br/>");
    }
  ...
  for (var ammo in ammunition) {
    ...
    switch (ammotype["class"]) {
      ...
      case "rockets":
        ...
        break;
      case "none":
        $(".standard.ammunition").after("<tr class='" + ammo + "'><td class='buyammo button'><strong>+</strong></td><td class='amountofammo'>0</td><td class='sellammo button'><em>-</em></td><td class='ammoname'>" + ammotype["name"] + "</td><td class='dammod'>" + ammotype["dammod"] + " " + ammotype["typemod"] + " " + ammotype["elemod"] + "</td><td class='apmod'>" + ammotype["apmod"] + "</td><td class='avail'>" + ammotype["avail"] + " " + ammotype["restrict"] + "</td><td class='cost'>" + ammotype["cost"] + "&#65509</td></tr>");
        break;
    }
  ...
  function abnormalAmmo(x, y) {
    $("." + x["class"]).after("<tr class='ammo'><td class='buyammo button'><strong>+</strong></td><td class='amountofammo'>0</td><td class='sellammo button'><em>-</em></td><td class='ammoname'>" + x["name"] + "</td><td class='dammod'>" + x["dammod"] + x["typemod"] + " " + x["elemod"] + "</td><td class='apmod'>" + x["apmod"] + "</td><td class='avail'>" + x["avail"] + " " + x["restrict"] + "</td><td class='cost'>" + x["cost"] + "&#65509</td></tr>");
  }
